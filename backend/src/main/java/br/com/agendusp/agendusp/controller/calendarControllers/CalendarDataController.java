...
public class CalendarDataController {
    ...
    public CalendarListResource addCalendarListResource(CalendarListResource calResource, String userId) {
        ...
        CalendarListResource calListResource = new CalendarListResource(); // Inicialmente, o usuário é o dono do
                                                                           // calendário
    ...
    public CalendarListResource getCalendarListResource(String calendarId, String userId) { // Método para obter um
    ...
    protected CalendarListResource updateCalendarListResource(String calendarId, CalendarListResource calListUserItem,
            String userId) {
    ...
    protected CalendarResource updateCalendarResource(String calendarId, CalendarResource calResource, String userId,
            String accessRole) {
    ...
    public CalendarListResource updateCalendar(String calendarId, CalendarListResource calListResource, String userId) {
        ...
        if (registeredCalListUserItem.getAccessRole() == "owner"
                || registeredCalListUserItem.getAccessRole() == "writer") {
    ...
    // @Override
    // public CalendarListResource patchCalendar(String calendarId, CalendarResource
    // calResource, String userId) {
    // CalendarListResource calListResource = userRepository
    // .findCalendarListResourceByUserIdAndCalendarId(userId, calendarId)
    // .orElseThrow(() -> new IllegalArgumentException(
    // "CalendarListResource com ID '" + calendarId + "' não encontrado para o
    // usuário de ID '"
    // + userId));
    // if (calListResource.getAccessRole() == "owner" ||
    // calListResource.getAccessRole() == "writer") {
    ...
    // throw new IllegalArgumentException(
    // "Acesso negado: o usuário não tem permissão para atualizar este
    // calendário.");
    // }
    ...
    public ArrayList<CalendarResource> getCalendars(String userId) throws Exception {
        ...
        for (CalendarListResource calListUserItem : calendarList) {
            CalendarResource calResource = calendarRepository
                    ...
                    .orElseThrow(
                            () -> new IllegalArgumentException("Calendário com ID '" + calListUserItem.getCalendarId()
                                    + "' não encontrado."));
    ...
    public void removeCalendar(String calendarId, String userId) {
        ...
        try {
            ...
            // Se o usuário tentar deletar um calendário que não existe, nada precisa ser
            // feito
