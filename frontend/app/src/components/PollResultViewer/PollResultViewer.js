...
import "./PollResultViewer.css";

const PollResultViewer = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      for (const dateTimeIntervalId of selectedIds) {
        await axios.post(
          `http://localhost:12003/poll/createEvent`,
          null,
          {
            params: {
              eventPollId: poll.id,
  ...
  return (
    <main id="vote-menu">
      <h2>Crie o evento com base no resultado da votação</h2>
      {eventName && (
        <div>
          <strong>Evento:</strong> {eventName}
        </div>
